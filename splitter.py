...
def split_file(infile=INPUT_FILE):
    ...
    if cursor >= total:
        cursor = 0
        save_cursor(0)
    chunk = read_chunk(infile, cursor, batch_size)
    if not chunk:
        cursor = 0
        save_cursor(0)
        chunk = read_chunk(infile, 0, batch_size)
    ...
    if percent > 100:
        percent = 100
    print(f"TOTAL={total} CURSOR={cursor} END={next_cursor} PART={len(chunk)} PROGRESS={percent}%")
    return OUTPUT_FILE
