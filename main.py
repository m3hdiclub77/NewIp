...
def main():
    ...
    print(f"[DEBUG] Merged {len(result)} IPs before sorting")

    result = sorted(result, key=lambda x: x[2], reverse=True)

    result = result[:4000]

    print(f"[DEBUG] Final {len(result)} IPs after cap")
    ...
    if result:
        print(f"[SUCCESS] Saved {len(result)} IPs to global_best.txt")
