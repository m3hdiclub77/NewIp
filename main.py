...
from scanned_ips import save_scanned_ips, reset_scanned_ips, scanned_count
...
def prepare():
    ...
    if should_update_bank():
        ...
        print("CLEAN START")
        clean_ips()
        print("RESETTING SCANNED IPS LIST (NEW BANK)")
        reset_scanned_ips()
        reset_cursor()
        print("BANK UPDATED - CURSOR RESET")
    
    scanned = scanned_count()
    print(f"CURRENT SCANNED IPS: {scanned}")
...
def main():
    ...
    parser.add_argument("--reset-scanned", action="store_true")
    ...
    if args.reset_scanned:
        reset_scanned_ips()
        reset_cursor()
        print("SCANNED IPS AND CURSOR RESET")
        return
    
    if args.status:
        total = 0
        try:
            with open("output/clean_ips.txt", "r", encoding="utf-8") as f:
                total = sum(1 for line in f if line.strip())
        except:
            pass
        scanned = scanned_count()
        print(f"TOTAL IPS: {total}")
        print(f"SCANNED IPS: {scanned}")
        print(f"REMAINING: {total - scanned if total > scanned else 0}")
        return
